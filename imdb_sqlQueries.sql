...
-- 21. Find all series that started in a specific year (e.g., 2020):
SELECT primaryTitle FROM series WHERE startYear = 2020;

-- 22.Retrieve all episodes of a particular series (replace 'tt0989125' with the series ID):
SELECT primaryTitle FROM episode WHERE seriesID = 'tt0989125';

-- 23. List all actors along with the titles they are known for:
...
-- 24.Find the total number of movies released each year:
SELECT releaseYear, COUNT(*) as totalMovies
FROM movie
GROUP BY releaseYear;

-- 25. Get all movies directed by a specific director (replace 'Director_Name' with the actual name):
...
-- 26.Find all writers who have worked on more than 5 movies:
...
-- 27. List series along with their genres:
SELECT s.primaryTitle, sg.genres
FROM series s
JOIN series_genre sg ON s.seriesID = sg.seriesID;

-- 28.Get all episodes in a particular season of a series (replace 'Series_ID' and 'Season_Number' with actual values):
SELECT primaryTitle
FROM episode
WHERE seriesID = 'tt0041038' AND seasonNumber = 1;


-- 29.Find all movies and series where a specific actor has acted (replace 'Actor_Name' with the actual name):
...
-- 30.List movies and their corresponding genres:
SELECT m.primaryTitle, mg.genres
FROM movie m
JOIN movie_genre mg ON m.movieID = mg.movieID;

-- 31.Retrieve all movies released in a particular year with a rating above a certain threshold (e.g., 8.0):
SELECT primaryTitle, averageRating
FROM movie
WHERE releaseYear = 2020 AND averageRating > 8.0;

-- 32. Find all series and the number of episodes in each:
SELECT s.primaryTitle, COUNT(e.episodeID) as NumberOfEpisodes
FROM series s
JOIN episode e ON s.seriesID = e.seriesID
GROUP BY s.seriesID;

-- 33.Get the primary and original titles of all episodes for a specific series (replace 'Series_ID' with actual value):
SELECT primaryTitle, originalTitle
FROM episode
WHERE seriesID = 'tt0041038';

-- 34.List all movies along with their directors:
...
-- 35.Retrieve all actors who have appeared in a specific movie (replace 'Movie_Title' with actual title):
...
-- 36.Find series that have an average rating higher than a specified value (e.g., 7.0):
SELECT primaryTitle, averageRating
FROM series
WHERE averageRating > 7.0;


-- 37.List all writers for a particular series (replace 'Series_Title' with actual title):
...
-- 38.Find all actors who were born in a specific year (e.g., 1980):
SELECT primaryName
FROM actor
WHERE birthYear = 1980;

-- 39.List all actors who have a birth year but no death year (i.e., potentially still alive):
SELECT primaryName
FROM actor
WHERE birthYear IS NOT NULL AND deathYear IS NULL;

-- 40.What is a typical runtime for movies in each genre?
...
-- 41. What genres are there? How many movies are there in each genre?
...
-- 42. How many movies are made in each genre each year?
...
-- 43. Series with More Episodes Than the Average Number of Episodes Per Series:
...
-- 44. Writers with More than Average Number of Votes for Their Movies:
...
-- 45. Actors Who Starred in Both Highest and Lowest Rated Movies:
...
-- 46. List actors who have acted in more than 5 movies:
SELECT a.primaryName
FROM actor a
WHERE a.actorID IN (
    SELECT ak.actorID
...
-- 47. Find Movies with Higher Ratings than Average of their Genre:
...
-- 48. Series with Most Episodes in a Given Year:
...
-- 49. Directors Who Have Worked in Both Movies and Series:
...
-- 50. Actors with Highest Average Rating Across Their Movies:
...
-- 51. Movies with Most Diverse Genres:
...
-- 52. Series with Increasing Popularity Over Years:
...
-- 53. Actors Who Have Acted in Movies with Above Average Runtime:
SELECT DISTINCT a.primaryName
FROM actor a
JOIN actor_known_for_titles akft ON a.actorID = akft.actorID
JOIN movie m ON akft.knownForTitles = m.movieID
...
-- 54. Most Common Genre Among Top Rated Movies:
SELECT mg.genres
FROM movie_genre mg
...
-- 55. Writers Who Have Worked in Both Highest and Lowest Rated Movies:
SELECT w.primaryName
FROM writer w
WHERE EXISTS (
    SELECT 1
...
-- 56.Find the average runtime of movies for each genre:
SELECT mg.genres, AVG(m.runtimeMinutes) as averageRuntime
FROM movie_genre mg
JOIN movie m ON mg.movieID = m.movieID
GROUP BY mg.genres;

-- 57. List actors who have only acted in movies with ratings above 7:
...
-- 58. Find directors who have directed more than 5 movies:
...
-- 59. Find series with more episodes than the average number of episodes per series: (same as 1)
...
-- 60. Find actors who have acted in both movies and series: 
SELECT a.actorID, a.primaryName
FROM actor a
WHERE EXISTS (
...
-- 61. Determine the average number of votes for movies by release year:
SELECT releaseYear, AVG(numVotes) as averageVotes
FROM movie
GROUP BY releaseYear;

-- 62. List series with a higher average rating than the average rating of all series:
